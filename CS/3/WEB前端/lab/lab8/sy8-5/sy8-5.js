function control() {
    ...
    for (i in table1Select) {
        if (i == index)
            table1Select[i].className = "";
        else
            table1Select[i].className = "hidden";
    }
    var table2 = document.getElementById("table2");
    var table2Select = table2.children;
    for (i in table2Select) {
        if (i == index)
            table2Select[i].className = "";
        else
            table2Select[i].className = "hidden";
    }
}
...
function putAll(a, b) {
    ...
    for (var x = len - 1; x >= 0; x--)
        table2Select[index].appendChild(table1Option[x]);
}